...
| Exhaustion carries **no sentinel** | `is_exhausted`, used by both `amerge` and `ataskify` | PEP 525 makes a `StopAsyncIteration` on a pull unambiguous, so the yielded type stays honestly `Task[T]` ([§5.4](./part-1-the-derivation.md#54-exhaustion-doesnt-need-a-sentinel)) |
