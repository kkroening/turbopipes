...
| **Cancels before it closes** | `amerge` and `ataskify`, each for its own pulls | `aclose()` cannot touch a source suspended inside its own body — and fails loudly from the cleanup path when you try ([§5.3](./part-1-the-derivation.md#53-aclose-will-not-touch-a-generator-thats-inside-its-own-body), [§5.4](./part-1-the-derivation.md#54-what-does-reach-a-running-generator-cancellation)) |
...
| `amerge` re-arms **after** the yield | `amerge` | halves the merge-wide produced-but-unconsumed figure, from two items to one; the per-source bound of one holds either way ([§5.5](./part-1-the-derivation.md#55-backpressure-survives-the-merge)) |
| Exhaustion carries **no sentinel** | `is_exhausted`, used by both `amerge` and `ataskify` | PEP 525 makes a `StopAsyncIteration` on a pull unambiguous, so the yielded type stays honestly `Task[T]` ([§7.3](./part-2-taking-it-apart.md#73-exhaustion-doesnt-need-a-sentinel)) |
